...
test('deferred routes never return their prototype component; authoritative routes retain theirs',async()=>{
 ...
 for(const path of ['/store/purchase','/STORE/PURCHASE','/%73tore/purchase','/dashboard','/reception','/customers','/cashier/buy-in','/chip-control','/cashier/cash-out','/cashier/reconciliation','/pit/tables','/slot-machines','/crm-gre','/fnb','/hr/roster','/reports/running-funds','/admin/business-date','/admin/system-lock','/admin/users','/Admin/Users/','/audit-logs'])assert.equal(TestScopeContent({path,children:child}),child)
})
