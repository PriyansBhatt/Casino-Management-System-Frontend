...
test('deferred routes never return their prototype component; authoritative routes retain theirs',async()=>{
 ...
 for(const path of ['/store/purchase','/accounts/bills','/analytics/management','/notifications','/demo/control-panel','/testing/checklist','/admin/users','/admin/roles','/admin/departments','/settings','/STORE/PURCHASE','/%73tore/purchase','/Admin/Users/','/audit-logs','/audit-logs/id']){
 ...
 for(const path of ['/dashboard','/reception','/customers','/cashier/buy-in','/chip-control','/cashier/cash-out','/cashier/reconciliation','/pit/tables','/slot-machines','/crm-gre','/fnb','/hr/roster','/reports/running-funds','/admin/business-date','/admin/system-lock'])assert.equal(TestScopeContent({path,children:child}),child)
})
