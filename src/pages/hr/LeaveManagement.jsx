...
const LeaveManagement = () => {
  ...
  const load = useCallback(async (nextFilters = filters, selectedId = selected?.requestId) => {
    ...
    try {
      ...
      setRequests(leaveRows); setTypes(leaveTypes); setStaff(staffRows); setDepartments(departmentRows)
      if (selectedId) setSelected(leaveRows.find((row) => row.requestId === selectedId) || null)
    } catch (requestError) {
      setRequests([]); setTypes([]); setStaff([]); setDepartments([]); setSelected(null)
      setError(requestError.message || 'Unable to load authoritative leave management data.')
    } finally { setLoading(false) }
  }, [filters, selected?.requestId])

  useEffect(() => { void load(emptyFilters, null) }, []) // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const openRequest = async (row) => {
    ...
    try { setSelected(await hrApi.getLeaveRequest(row.requestId)) }
    catch (requestError) { showToast({ type: 'error', title: 'Request unavailable', message: requestError.message || 'Unable to load this leave request.' }) }
  }
  ...
  const submitAction = async (event) => {
    ...
    setSaving(true)
    try {
      if (action.type === 'approve') await hrApi.approveLeaveRequest(selected.requestId, { remarks: reason || null })
      if (action.type === 'reject') await hrApi.rejectLeaveRequest(selected.requestId, { reason })
      if (action.type === 'cancel') await hrApi.cancelLeaveRequest(selected.requestId, { reason })
      const selectedId = selected.requestId
      setAction(null)
      await load(filters, selectedId)
      showToast({ type: 'success', title: 'Leave request updated', message: 'The authoritative lifecycle state was refreshed.' })
    } catch (requestError) {
      showToast({ type: 'error', title: 'Action rejected', message: requestError.message || 'The leave action could not be completed.' })
  ...
  const saveType = async (event) => {
    ...
    try {
      ...
      if (typeEditor.record) { delete payload.code; await hrApi.updateLeaveType(typeEditor.record.id, payload) }
      else await hrApi.createLeaveType({ ...payload, code: form.code.trim().toUpperCase() })
      setTypeEditor(null); await load(filters, selected?.requestId)
      showToast({ type: 'success', title: 'Leave type saved', message: 'The authoritative leave-type list was refreshed.' })
    } catch (requestError) {
      showToast({ type: 'error', title: 'Save failed', message: requestError.message || 'The leave type was rejected.' })
    } finally { setSaving(false) }
  }

  return <div className="space-y-6">
    <PageHeader title="Leave Management" description="Review full-day leave requests and maintain authoritative leave types." actions={<Button type="button" variant="outline" onClick={() => load(filters, selected?.requestId)} disabled={loading}>Refresh</Button>} />
    <div className="flex gap-2 rounded-xl bg-slate-100 p-1"><Tab active={tab === 'requests'} onClick={() => setTab('requests')}>Leave Requests</Tab><Tab active={tab === 'types'} onClick={() => setTab('types')}>Leave Types</Tab></div>
    {tab === 'requests' && <Card><form onSubmit={applyFilters} className="grid gap-3 md:grid-cols-4 lg:grid-cols-7"><Field label="From"><input type="date" value={filters.startDate} onChange={(e) => setFilters({ ...filters, startDate: e.target.value })} className={inputClass} /></Field><Field label="To"><input type="date" value={filters.endDate} onChange={(e) => setFilters({ ...filters, endDate: e.target.value })} className={inputClass} /></Field><Field label="Employee"><select value={filters.staffProfileId} onChange={(e) => setFilters({ ...filters, staffProfileId: e.target.value })} className={inputClass}><option value="">All employees</option>{staff.map((row) => <option key={row.staffProfileId} value={row.staffProfileId}>{row.fullName || row.username} · {row.employeeCode}</option>)}</select></Field><Field label="Department"><select value={filters.departmentId} onChange={(e) => setFilters({ ...filters, departmentId: e.target.value })} className={inputClass}><option value="">All departments</option>{departments.map((row) => <option key={row.id} value={row.id}>{row.name}</option>)}</select></Field><Field label="Leave Type"><select value={filters.leaveTypeId} onChange={(e) => setFilters({ ...filters, leaveTypeId: e.target.value })} className={inputClass}><option value="">All types</option>{types.map((row) => <option key={row.id} value={row.id}>{row.name}</option>)}</select></Field><Field label="Status"><select value={filters.status} onChange={(e) => setFilters({ ...filters, status: e.target.value })} className={inputClass}><option value="">All statuses</option>{STATUSES.map((value) => <option key={value}>{value}</option>)}</select></Field><div className="flex items-end"><Button type="submit" disabled={loading}>Apply</Button></div></form></Card>}
    {loading && <Card><Loading message="Loading authoritative leave management data..." size="sm" /></Card>}
    {!loading && error && <ErrorState title="Leave management unavailable" description={error} onRetry={() => load(filters, null)} />}
    ...
    {selected && <RequestDetail row={selected} onClose={() => setSelected(null)} onAction={(type) => setAction({ type, reason: '' })} />}
    {action && <Overlay><Header title={`${action.type[0].toUpperCase()}${action.type.slice(1)} Leave Request`} onClose={() => setAction(null)} /><p className="mt-2 text-sm text-slate-600">{selected.staff?.fullName || selected.staff?.username} · {selected.startDate} to {selected.endDate}. The backend will enforce the current lifecycle and roster-conflict rules.</p><form onSubmit={submitAction} className="mt-5"><Field label={action.type === 'approve' ? 'Remarks (optional)' : 'Reason'}><textarea required={action.type !== 'approve'} maxLength="500" rows="4" value={action.reason} onChange={(e) => setAction({ ...action, reason: e.target.value })} className={inputClass} /></Field><p className="mt-1 text-right text-xs text-slate-500">{action.reason.length}/500</p><Actions saving={saving} onClose={() => setAction(null)} label={`Confirm ${action.type}`} danger={action.type !== 'approve'} /></form></Overlay>}
