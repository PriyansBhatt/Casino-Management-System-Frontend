...
const AttendanceManagement = () => {
  ...
  const clearSelection = () => {
    safety.invalidate('history', 'scope')
    setSelected(null); setHistory([]); setHistoryError(''); setHistoryLoading(false); setCorrection(null)
  }
  const changeDate = date => {
    safety.invalidate('report', 'scope'); clearSelection()
    setRefreshWarning(''); setBusinessDate(date); setRecords([]); setLoaded(false); setLoading(false); setError(''); setConfirmation(null)
  }
  const loadReport = async (date = businessDate) => {
    if (!date || !safety.live()) return
    const current = safety.begin('report')
    setRefreshWarning(''); setConfirmation(null)
    clearSelection()
    setLoading(true); setError(''); setRecords([]); setLoaded(true)
    try {
      const rows = await attendanceApi.getReport(date)
      if (!current()) return
      setRecords(rows); return true
    } catch (requestError) {
      if (!current()) return
      setError(requestError.message || 'Unable to load the authoritative attendance report.'); return false
    } finally { if (current()) setLoading(false) }
  }
  useEffect(() => { void loadReport(businessDate) }, []) // eslint-disable-line react-hooks/exhaustive-deps

  const loadHistory = async attendanceId => {
    const current = safety.begin('history')
    setHistory([]); setHistoryError(''); setHistoryLoading(true)
    try {
      const rows = await attendanceApi.getAttendanceCorrections(attendanceId)
      if (!current()) return
      setHistory(rows); return true
    } catch (requestError) {
      if (!current()) return
      setHistoryError(requestError.message || 'Unable to load correction history.'); return false
    } finally { if (current()) setHistoryLoading(false) }
  }
  const selectRecord = record => {
    safety.invalidate('scope')
    setSelected(record); setCorrection(null)
  ...
  const prepareCorrection = (event) => {
    ...
    setConfirmation(freezeHrIntent({ attendanceId: selected.attendanceId, operation: 'correction', payload, record: selected, date: businessDate }))
  }

  const submitCorrection = () => {
    const intent = confirmation, current = safety.capture('scope')
    return safety.submit(intent, frozen => attendanceApi.createAttendanceCorrection(frozen.attendanceId, frozen.payload), {
      busy: setSubmitting,
      success: () => { setCorrection(null); setConfirmation(null); setRefreshWarning(''); showToast({ type: 'success', title: 'Attendance corrected', message: 'The attendance correction was saved.' }) },
      failure: error => showToast({ type: 'error', title: 'Correction failed', message: error.message || 'The attendance correction was rejected.' }),
      refresh: () => current() && intent.date === businessDate ? loadReport(intent.date) : undefined,
      warning: () => setRefreshWarning('Attendance updated, but refreshed data could not be loaded.'),
    })
  }
  ...
  return <div className="space-y-6">
    ...
    <Card>
      <form onSubmit={(event) => { event.preventDefault(); void loadReport(businessDate) }} className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <Field label="Attendance Business Date"><input required type="date" value={businessDate} onChange={event => changeDate(event.target.value)} className={inputClass} /></Field>
    ...
    {!loading && error && <ErrorState title="Attendance report unavailable" description={error} onRetry={() => loadReport(businessDate)} />}
    {!loading && !error && loaded && <Card>
      <div className="grid gap-3 border-b border-slate-200 pb-5 md:grid-cols-3">
        <Field label="Search loaded report"><input value={search} onChange={event => { clearSelection(); setSearch(event.target.value) }} placeholder="Employee name or username" className={inputClass} /></Field>
        <Field label="Attendance Status"><select value={statusFilter} onChange={event => { clearSelection(); setStatusFilter(event.target.value) }} className={inputClass}><option value="">All statuses</option><option>OPEN</option><option>CLOSED</option></select></Field>
        <Field label="Schedule Classification"><select value={classificationFilter} onChange={event => { clearSelection(); setClassificationFilter(event.target.value) }} className={inputClass}><option value="">All classifications</option>{CLASSIFICATIONS.map((value) => <option key={value}>{value}</option>)}</select></Field>
      </div>
    ...
    {selected && <AttendanceDetail record={selected} history={history} historyLoading={historyLoading} historyError={historyError} onRetryHistory={() => loadHistory(selected.attendanceId)} onClose={clearSelection} onCorrect={openCorrection} />}
    ...
    <ConfirmDialog isOpen={Boolean(confirmation)} title="Confirm audited Attendance correction" description={confirmation ? confirmationText(confirmation.record, confirmation.payload) : ''} confirmLabel="Apply Correction" variant="warning" isLoading={submitting} onCancel={() => setConfirmation(null)} onConfirm={submitCorrection} />
  </div>
}
...
const confirmationText = (record, payload) => [`Employee: ${record.employee?.fullName || record.employee?.username || 'Unavailable'}`, `Business Date: ${record.businessDate}`, `Type: ${friendly(payload.type)}`, payload.checkInAt ? `Proposed check-in: ${formatAttendanceTimestamp(payload.checkInAt)} (current: ${formatAttendanceTimestamp(record.checkInAt)})` : '', payload.checkOutAt ? `Proposed check-out: ${formatAttendanceTimestamp(payload.checkOutAt)} (current: ${formatAttendanceTimestamp(record.checkOutAt)})` : '', `Reason: ${payload.reason}`, 'This audited correction will update authoritative Attendance history.'].filter(Boolean).join('\n')
...
export default function HrPage() { const { user } = useAuth(); return <AttendanceManagement key={hrActorKey(user)} /> }
