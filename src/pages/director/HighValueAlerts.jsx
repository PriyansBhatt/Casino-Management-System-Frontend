...
import { getHighValueAlerts, markAlertReviewed } from '../../api/directorApi'
...
const highValueTypes = [ALERT_TYPES.HIGH_VALUE_BUY_IN, ALERT_TYPES.HIGH_VALUE_CASH_OUT]
...
const HighValueAlerts = () => {
  ...
  const loadAlerts = async () => {
    ...
    try {
      const data = await getHighValueAlerts({ businessDate, status, type })
  ...
  const handleMarkReviewed = async () => {
    ...
    try {
      ...
      safeLogAuditEvent({
        ...
        description: `High-value alert ${selectedAlert.id} reviewed.`,
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="Review cashier buy-in and cash-out alerts at or above the high-value threshold."
      />

      <Card className="border-red-200 bg-red-50">
        <p className="text-sm font-semibold text-red-800">Current threshold: NPR 100,000</p>
      </Card>
      ...
      <Card className="overflow-hidden p-0">
        {isLoading && <p className="p-6 text-sm text-gray-600">Loading high-value alerts...</p>}
        {!isLoading && !error && alerts.length === 0 && (
          <div className="p-6 text-center">
            <p className="font-semibold text-gray-900">No open alerts for selected business date.</p>
            <p className="mt-1 text-sm text-gray-600">Create a cashier transaction over NPR 100,000 to generate one.</p>
          </div>
        )}
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  {['Alert Reference', 'Type', 'Customer Code', 'Customer Name', 'Amount', 'Transaction Reference', 'Business Date', 'Status', 'Created At', 'Notes', 'Actions'].map((header) => (
              ...
              <tbody className="divide-y divide-gray-100 bg-white">
                {alerts.map((alert) => (
                  <tr key={alert.id} className={alert.status === ALERT_STATUSES.OPEN ? 'bg-amber-50/40' : 'hover:bg-gray-50'}>
                    ...
                    <td className="whitespace-nowrap px-4 py-3 text-sm text-gray-700">{alert.customerCode || 'Not available'}</td>
                    <td className="whitespace-nowrap px-4 py-3 text-sm text-gray-700">{alert.customerName || 'Not available'}</td>
                    <td className="whitespace-nowrap px-4 py-3 text-sm font-semibold text-gray-900">{formatAmount(alert.amount)}</td>
                    <td className="whitespace-nowrap px-4 py-3 text-sm text-gray-700">{alert.reference || 'Not available'}</td>
...
export default HighValueAlerts
