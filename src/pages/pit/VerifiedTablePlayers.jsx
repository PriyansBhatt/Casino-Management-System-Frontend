...
const VerifiedTablePlayers = ({ tableId, onSummaryChange }) => {
  ...
  const [customerCustody, setCustomerCustody] = useState(null)
  const [tableCustody, setTableCustody] = useState(null)
  const [custodyMode, setCustodyMode] = useState(null)
  const [custodyQuantities, setCustodyQuantities] = useState(emptyQuantities)
  const [error, setError] = useState('')
  const [busy, setBusy] = useState(false)
  const requestRef = useRef({ signature: null, key: null })
  const custodyRequestRef = useRef({ signature: null, key: null })
  ...
  const selectPlayer = async (player, type = null) => {
    ...
    setCustomerCustody(null); setTableCustody(null)
    try {
      const [history, customerInventory, tableInventory] = await Promise.all([
        pitApi.getVerifiedGamingResults(player.customerSessionId),
        chipCustodyApi.getCustomerSessionInventory(player.customerSessionId),
        chipCustodyApi.getTableInventory(tableId),
      ])
      setResults(history.filter((item) => item.assignmentId === player.assignmentId))
      setCustomerCustody(customerInventory); setTableCustody(tableInventory)
    } catch (err) { setError(getErrorMessage(err)); setResults([]) }
  }

  const openCustodyAction = async (player, mode) => {
    setError('')
    await selectPlayer(player)
    setCustodyMode(mode)
    setCustodyQuantities(emptyQuantities())
    custodyRequestRef.current = { signature: null, key: null }
  }
  ...
  const custodyTotal = useMemo(() => denominations.reduce(
    (value, denomination) => value + denomination * Number(custodyQuantities[denomination] || 0), 0),
  [custodyQuantities])

  const submitCustodyAction = async () => {
    if (!canMutate || !selected) { setError('This Pit Table is closed or casino operations are locked.'); return }
    const positive = Object.fromEntries(denominations.filter((value) => custodyQuantities[value] > 0)
      .map((value) => [value, Number(custodyQuantities[value])]))
    if (custodyMode !== 'LEAVE' && custodyTotal <= 0) {
      setError('Enter at least one chip denomination quantity.'); return
    }
    const signature = JSON.stringify({ assignmentId: selected.assignmentId, custodyMode, denominations: positive })
    if (custodyRequestRef.current.signature !== signature) {
      if (!globalThis.crypto?.randomUUID) { setError('Secure request processing is unavailable in this browser.'); return }
      custodyRequestRef.current = { signature, key: globalThis.crypto.randomUUID() }
    }
    setBusy(true); setError('')
    try {
      const payload = { denominations: positive, idempotencyKey: custodyRequestRef.current.key }
      if (custodyMode === 'TO_TABLE') {
        await chipCustodyApi.moveCustomerChipsToTable(tableId, selected.customerSessionId, payload)
      } else if (custodyMode === 'TO_CUSTOMER') {
        await chipCustodyApi.returnTableChipsToCustomer(tableId, selected.customerSessionId, payload)
      } else {
        await pitApi.leavePlayer(tableId, selected.assignmentId, payload)
      }
      if (custodyMode === 'LEAVE') {
        await loadPlayers(); setSelected(null); setResults([])
        showToast({ type: 'success', title: 'Custody Settled & Player Left', message: selected.customerName })
      } else {
        const [customerInventory, tableInventory] = await Promise.all([
          chipCustodyApi.getCustomerSessionInventory(selected.customerSessionId),
          chipCustodyApi.getTableInventory(tableId),
        ])
        setCustomerCustody(customerInventory); setTableCustody(tableInventory)
        showToast({ type: 'success', title: 'Physical Chips Transferred', message: money(custodyTotal) })
      }
      setCustodyMode(null); setCustodyQuantities(emptyQuantities())
      custodyRequestRef.current = { signature: null, key: null }
    } catch (err) {
      const message = getErrorMessage(err); setError(message)
      showToast({ type: 'error', title: 'Chip Transfer Failed', message })
    } finally { setBusy(false) }
  }
  ...
  return <div className="space-y-5">
    ...
    <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
      {players.map((player) => { const totals = playerSummaries[player.assignmentId] || { WIN: 0, LOSS: 0 }; return <article key={player.assignmentId} className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        ...
        <div className="p-4"><div className="grid grid-cols-2 gap-3 text-sm"><Info label="Badge" value={player.badge || 'Unavailable'}/><Info label="Table" value={table?.tableCode || 'Loading...'}/><Info label="Joined" value={player.joinedAt ? new Date(player.joinedAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : 'Unavailable'}/><Info label="Status" value={table?.status || 'Unavailable'}/><Info label="Verified Wins" value={money(totals.WIN)} tone="green"/><Info label="Verified Losses" value={money(totals.LOSS)} tone="red"/></div>
          <div className="mt-4 grid grid-cols-2 gap-2"><button type="button" onClick={() => selectPlayer(player, 'WIN')} disabled={!canMutate} className="rounded-xl bg-emerald-600 px-3 py-2.5 text-sm font-black text-white hover:bg-emerald-500 disabled:opacity-40">Record Win</button><button type="button" onClick={() => selectPlayer(player, 'LOSS')} disabled={!canMutate} className="rounded-xl bg-red-600 px-3 py-2.5 text-sm font-black text-white hover:bg-red-500 disabled:opacity-40">Record Loss</button><button type="button" onClick={() => openCustodyAction(player, 'TO_TABLE')} disabled={!canMutate} className="rounded-xl border border-sky-300 bg-sky-50 px-3 py-2.5 text-sm font-black text-sky-800 disabled:opacity-40">Move Chips to Table</button><button type="button" onClick={() => openCustodyAction(player, 'TO_CUSTOMER')} disabled={!canMutate} className="rounded-xl border border-indigo-300 bg-indigo-50 px-3 py-2.5 text-sm font-black text-indigo-800 disabled:opacity-40">Return to Customer</button><button type="button" onClick={() => selectPlayer(player)} className="rounded-xl border border-slate-200 px-3 py-2.5 text-sm font-black text-slate-700 hover:bg-slate-50">View Details</button><button type="button" onClick={() => openCustodyAction(player, 'LEAVE')} disabled={busy || !canMutate || player.status !== 'ACTIVE'} className="rounded-xl border border-amber-300 bg-amber-50 px-3 py-2.5 text-sm font-black text-amber-800 disabled:opacity-50">Settle &amp; Leave</button></div>
        </div></article> })}
    ...
    {selected && <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm"><div className="flex items-center justify-between border-b border-slate-200 p-4"><div><h2 className="font-black text-slate-950">Player Position &amp; Result History</h2><p className="text-sm text-slate-500">{selected.customerName} · {selected.customerCode}</p></div><button type="button" onClick={() => setSelected(null)} className="text-sm font-bold text-slate-500">Close</button></div><div className="grid gap-3 border-b border-slate-200 bg-slate-50 p-4 sm:grid-cols-3"><CustodyValue label="Customer Physical Chips" value={customerCustody ? money(customerCustody.totalValue) : 'Loading...'}/><CustodyValue label="Table Physical Chips" value={tableCustody ? money(tableCustody.totalValue) : 'Loading...'}/><CustodyValue label="Customer Financial Position" value={money(selected.calculatedChipPosition)}/></div><p className="border-b border-slate-100 px-4 py-3 text-xs font-semibold text-slate-500">Physical custody and financial position are separate authoritative values.</p><div className="overflow-x-auto"><table className="min-w-[760px] w-full text-left text-sm"><thead className="bg-slate-50 text-xs uppercase tracking-wide text-slate-500"><tr><th className="px-4 py-3">Time</th><th className="px-4 py-3">Customer</th><th className="px-4 py-3">Result</th><th className="px-4 py-3">Denominations</th><th className="px-4 py-3">Amount</th><th className="px-4 py-3">Recorded By</th><th className="px-4 py-3">Status</th></tr></thead><tbody className="divide-y divide-slate-100">{results.map((item) => <tr key={item.id}><td className="px-4 py-3">{new Date(item.createdAt).toLocaleTimeString()}</td><td className="px-4 py-3 font-bold">{selected.customerName}</td><td className="px-4 py-3"><ResultBadge type={item.resultType}/></td><td className="px-4 py-3">{summary(item.denominations)}</td><td className="px-4 py-3 font-black">{money(item.amount)}</td><td className="px-4 py-3">{item.createdBy?.username || 'User unavailable'}</td><td className="px-4 py-3"><span className="rounded-full bg-emerald-50 px-2 py-1 text-xs font-black text-emerald-700">VERIFIED</span></td></tr>)}</tbody></table>{results.length === 0 && <p className="p-6 text-center text-sm text-slate-500">No verified results recorded for this table assignment.</p>}</div></section>}

    {custodyMode && selected && <div className="fixed inset-0 z-50 flex justify-end bg-slate-950/40" onMouseDown={() => !busy && setCustodyMode(null)}><aside onMouseDown={(event) => event.stopPropagation()} className="h-full w-full max-w-lg overflow-y-auto bg-white shadow-2xl"><div className="flex items-start justify-between border-b border-slate-200 p-5"><div><p className="text-xs font-black uppercase tracking-wider text-sky-600">Physical Chip Custody</p><h2 className="mt-1 text-2xl font-black text-slate-950">{custodyMode === 'TO_TABLE' ? 'Move Chips to Table' : custodyMode === 'TO_CUSTOMER' ? 'Return Chips to Customer' : 'Settle & Leave Table'}</h2><p className="mt-1 text-sm text-slate-500">{selected.customerName} · {selected.sessionCode}</p></div><button type="button" onClick={() => setCustodyMode(null)} disabled={busy} className="rounded-lg border px-3 py-2 text-sm font-bold">Cancel</button></div><div className="space-y-5 p-5"><div className="grid grid-cols-2 gap-3"><CustodyValue label="Customer Physical" value={customerCustody ? money(customerCustody.totalValue) : 'Unavailable'}/><CustodyValue label="Table Physical" value={tableCustody ? money(tableCustody.totalValue) : 'Unavailable'}/><CustodyValue label="Financial Position" value={money(selected.calculatedChipPosition)}/><CustodyValue label="Business Date" value={selected.businessDate}/></div>{custodyMode === 'LEAVE' && <p className="rounded-xl border border-amber-200 bg-amber-50 p-3 text-sm font-semibold text-amber-800">Enter only the chips physically returned to the customer during settlement. Any chips not returned remain in authoritative table custody. Confirming this step ends the assignment atomically.</p>}<div><p className="mb-3 text-sm font-black text-slate-800">{custodyMode === 'TO_TABLE' ? 'Chips leaving customer custody' : 'Chips leaving table custody'}</p><div className="space-y-2">{denominations.map((denomination) => { const available = custodyMode === 'TO_TABLE' ? customerCustody?.denominations?.[denomination] : tableCustody?.denominations?.[denomination]; return <div key={denomination} className="flex items-center justify-between rounded-xl border border-slate-200 p-3"><div><p className="font-black">{money(denomination)}</p><p className="text-xs text-slate-500">Available: {Number(available || 0)}</p></div><input type="number" min="0" step="1" value={custodyQuantities[denomination]} onChange={(event) => { setCustodyQuantities((current) => ({ ...current, [denomination]: Math.max(0, Math.trunc(Number(event.target.value) || 0)) })); custodyRequestRef.current = { signature: null, key: null } }} className="h-10 w-24 rounded-lg border text-center font-black"/></div>})}</div></div><div className="rounded-2xl bg-slate-950 p-5 text-white"><p className="text-sm font-bold">Calculated Physical Transfer</p><p className="mt-1 text-3xl font-black">{money(custodyTotal)}</p></div>{error && <p className="rounded-xl bg-red-50 p-3 text-sm font-bold text-red-700">{error}</p>}<button type="button" onClick={submitCustodyAction} disabled={busy || isSystemLocked || (custodyMode !== 'LEAVE' && custodyTotal <= 0)} className="h-12 w-full rounded-xl bg-sky-700 text-sm font-black text-white disabled:opacity-50">{busy ? 'Processing...' : custodyMode === 'LEAVE' ? 'Confirm Settlement & Leave' : 'Confirm Physical Transfer'}</button></div></aside></div>}
...
const CustodyValue = ({ label, value }) => <div className="rounded-xl border border-slate-200 bg-white p-3"><p className="text-xs font-semibold text-slate-500">{label}</p><p className="mt-1 break-words font-black text-slate-950">{value || 'Unavailable'}</p></div>
